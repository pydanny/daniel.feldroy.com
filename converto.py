...
def _(Path, subprocess):
    def list_files():
        ...
        for path in Path('.').glob('**/*.ipynb'):
            ...
            subprocess.run(['marimo', 'convert', str(path), '-o', str(marimo_path)], capture_output=True, text=True)
            print(cmd)
            cmd = f'marimo export md {marimo_path} -o {md_path}'
            subprocess.run(['marimo', 'export', 'md', str(marimo_path), '-o', str(md_path)], capture_output=True, text=True)     
            print(cmd)
