...
def mailing_list():
    ...
    return Layout(
        ...
        air.Article(
            ...
            air.P(
                air.Small(
                    "By subscribing you agree to receive periodic emails from feldroy.com, which is just me and my wife/partner Audrey."
                )
            ),
            id="form",
        ),
...
def send_subscription_email(email, initial=True):
    if initial:
        post_email(
            email,
            subject="A confirmation email",
            text="",
            html=air.Tag(air.H1("Another email confirmation")).render(),
            mailserver="mailgun",
        )
    else:
        post_email(
            email,
            subject="We're sending another confirmation email ",
            text="",
            html=air.Tag(air.H1("Another email confirmation")).render(),
            mailserver="mailgun",
        )
...
async def save_subscription(email):
    ...
    if subscription is None:
        ...
        send_subscription_email(email)
        print(
            "[red bold]In the system already, let's send them the email again[/red bold]"
        )
...
async def subscribe(request: air.Request, background_tasks: air.BackgroundTasks):
    ...
    return air.Article(
        air.P(
            "Check your email (and your spam folder) for a confirmation email which should arrive in the next few minutes."
        ),
        id="form",
        hx_swap_oob="true",
    )
