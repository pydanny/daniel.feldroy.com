...
def render_code_output(cell, lang="python", render_md=render_md):
    ...
    def _join_strip_list(lst):
        return "".join(strip_list(lst))
    ...
    for output in cell["outputs"]:
        if output["output_type"] in ("execute_result", "display_data"):
            ...
            if "text/html" in data.keys():
                ...
                res.append(
                    NotStr(render_md(_join_strip_list(data["text/markdown"][1:-1])))
                )
...
def list_posts(
    ...
    for post in pathlib.Path(".").glob(f"{posts_dirname}/**/*.ipynb"):
        ...
        data["cls"] = "notebook"
        if content:
            data["content"] = render_nb(
                post, cls="", fm_fn=lambda x: "", out_fn=render_code_output
            )
        posts.append(data)
    # Fetch markdown
        ...
        (
            ...
            Footer(
                ...
                P(
                    ...
                    " | ",
                    "Feeds: ",
                    A("All", href="/feeds/atom.xml"),
                    NotStr(", "),
                    A("Python", href="/feeds/python.atom.xml"),
                    NotStr(", "),
                    A("TIL", href="/feeds/til.atom.xml"),
                ),
    ...
    return (
        ...
        Section(
            ...
            P(
                metadata.get("author", ""),
                Br(),
                Small(Time(format_datetime(convert_dtstr_to_dt(date)))),
            ),
...
def get(slug: str):
    ...
    return Layout(
        ...
        Socials(
            ...
            image="https://daniel.feldroy.com"
            + metadata.get("image", default_social_image),
        ),
        Section(
            H1(metadata["title"]),
...
def fitness():
    ...
    for month, rows in dates.items():
        ...
        charts.insert(
            1, Script(f"Plotly.newPlot('{chart_name}', {fitness}, {layout}, {config});")
        )
...
def stories():
    ...
    for story in stories:
        story = str(story)[5:-3]
        links.append(Li(A(href=story)(story)))
    return Layout(
        Title("Stories"),
        Socials(
            site_name="https://daniel.feldroy.com",
